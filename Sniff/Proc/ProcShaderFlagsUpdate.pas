...
constructor TProcShaderFlagsUpdate.Create(aManager: TProcManager);
  ...
  function GetFlagsList(aFlags: TdfDef): TStringList;
  begin
    Result := TStringList.Create;
    with TdfFlagsDef(aFlags) do try
      for var i := 0 to Pred(ValuesMapCount) do
        Result.Add(Values[i]);
    finally
      Free;
    end;
...
begin
  ...
  slFO3Flags := GetFlagsList(wbBSShaderFlags('', '', []));
  slFO3Flags2 := GetFlagsList(wbBSShaderFlags2('', '', []));

  slTES5Flags := GetFlagsList(wbSkyrimShaderPropertyFlags1('', '', []));
  slTES5Flags2 := GetFlagsList(wbSkyrimShaderPropertyFlags2('', '', []));

  slFO4Flags := GetFlagsList(wbFallout4ShaderPropertyFlags1('', '', []));
  slFO4Flags2 := GetFlagsList(wbFallout4ShaderPropertyFlags2('', '', []));
end;
